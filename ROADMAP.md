...
-   Hook: `RegisteredUserController` after user create; grant via `user_items` / `User::items()`
-   Catalog: [`database/seeders/ItemSeeder.php`](database/seeders/ItemSeeder.php) currently only Scorpion + White-Modifier Stone
-   **Audits:** [reference/FEATURE-AUDIT.md](reference/FEATURE-AUDIT.md), [reference/DATA-LAYER-AUDIT.md](reference/DATA-LAYER-AUDIT.md)

### Acceptance Criteria

-   [ ] New registration grants the full welcome package quantities to `user_items`
-   [ ] Missing catalog items are seeded or creation fails loudly in tests
-   [ ] Pest feature test covers grant on register; no duplicate grant on email verify
-   [ ] Grant is logged or otherwise auditable for support
