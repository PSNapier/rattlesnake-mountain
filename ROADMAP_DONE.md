...
- **Admin:** "Announcements" card on the CMS tab at `/admin`. Each row shows the title, publish date and Published, Scheduled or Draft.
- **Admin:** "New announcement" or Edit on a row. A dialog with Title, Publish at (blank saves a draft) and a Body editor with bold, links, lists and images, saved with "Save announcement".
- **Admin:** Delete on a row. The post leaves `/` and `/news` at once and staff can recover it.
- **Visitor:** `/`. Reads the newest announcement, clamped when long, with "Read more news".
- **Visitor:** `/news`. Reads staff intro copy above every published announcement in full, newest first, 10 per page with Newer and Older links.
