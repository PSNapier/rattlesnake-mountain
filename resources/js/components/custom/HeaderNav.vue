...
<template>
	<nav
		...
		<ul>
			<a :href="linkDict.HOME.path">{{ linkDict.HOME.label }}</a>
		</ul>
		<ul class="group relative">
			<a :href="linkDict.GETTING_STARTED.path">{{
				linkDict.GETTING_STARTED.label
			}}</a>
			<div
				class="bg-shakespeare-100 text-shakespeare-400 absolute right-0 left-0 hidden min-w-max rounded-b-lg p-2 font-sans text-lg font-normal group-hover:block">
				<a href="#"
					><li
				...
				>
				<a href="#"
					><li class="hover:text-shakespeare-300 pt-2">
						Test
					</li></a
				>
			</div>
		...
		<ul>
			<a :href="linkDict.WILDLIFE.path">{{
				linkDict.WILDLIFE.label
			}}</a>
		</ul>
		<ul>
			<a :href="linkDict.CONTACT_US.path">{{
				linkDict.CONTACT_US.label
			}}</a>
		</ul>
